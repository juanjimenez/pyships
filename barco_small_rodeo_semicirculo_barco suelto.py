...
# A boom instance
cd5 = boom.cadena(50,np.array([0,2,1000]),np.array([1,0])) #to moore the tip,np.array([1,0]))
cd5.L = 0.415
...
ffv = 1.6 #fijamos la potencia transmitida al motor en lugar de controlar la velocidad
...
trecord = delta * 2000. #it should be a multiple of the integration step...or not
...
#            print norm(bd.rlink[1][1] - bd.rlink[0][1])
            ...
            ####dibujo iterativo no usar en simulaciones largas#########
            #print bd.thewj * 180 /np.pi, bd.cstb(bd.vb)
            pl.plot(bd.rlink[1][1][0],bd.rlink[1][1][1],'ob')
            bd.dibujar()
            cd5.dibujar()
