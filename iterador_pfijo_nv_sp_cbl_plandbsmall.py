...
np.array([4 + bd.pb[0],1 + bd.pb[1],4,1]),\
np.array([8,10.5,4,-1]),\
np.array([-4,10.5,4,-1]),\
np.array([-12,4,4,1]),\
np.array([-12,0,0,1])]
...
t= 0
tiempo = 10. #600. #total simulation time-> it cannot be an INTEGERRRRR
...
#           
    ...
    if i[0][2] == 0: #start and end waypoints has not to be turned around.
        ant = i[1]
        continue
...
#            print norm(bd.rlink[1][1] - bd.rlink[0][1])
            print bd.thewj * 180 /np.pi
            pl.plot(bd.rlink[1][1][0],bd.rlink[1][1][1],'o')
            bd.dibujar()
            pl.pause(1)
            step += 1
        if t>= tiempo:
