...
Formationtemp= Formation.groupby(['time_sec']).mean()
Formationtemp= Formation.reset_index()
...
wp = [np.array([0.,0.,0,-1]),\
np.array([-27.25,35.,14.,-1]),\
np.array([-27.25,21.5,12,-1]),\
np.array([-27.25,35.,14.,-1])]
...
ini2 = 940
fin2 = 1665
errorlat = Formationtemp['errorLateral']\
[(Formation.time_sec>=Formation.time_sec[ini2])&(Formation.time_sec<=Formation.time_sec[fin2])].get_values() 
tiempo = Formationtemp['time_sec'][ini2:fin2+1].get_values()

pl.figure(2)
pl.plot(tiempo-tiempo[0],errorlat)
