...
bd.mua2 = 1.7#9.7577 #caveat in the equations damping is bd.mua*bd.ls*bd.wb**2 
bd.mut2 = 2.23#4. #2.226
bd.mut = 0.1#0.1
bd.mul2= 1.22#1.219
bd.mul = 0.01
bd.krpid = [2,0.5,0]
...
bd.theta = read_log_bkp.heading[0]
#bd.thewj = 20 * pi /180
...
#bd.pb = np.array([50.,0.])
bd.pb = np.array(read_log_bkp.datos_m).T[0]#np.array([0.,0])
#vset = 1 #0.5 #usv speed setpoint
ffv = 4.5 #fijamos la potencia transmitida al motor en lugar de controlar la velocidad
...
t= 0.
tiempo =66 #ojo 80 solo para corriente a favor
#tiempo= 130. #600. #total simulation time-> it cannot be an INTEGERRRRR
#size of the arrays to save experimente results....
trecord = delta * 100000. #it should be a multiple of the integration step...or not
tpintar = trecord * 10 #* 20.
...
while (t <= tiempo) & (nvueltas < maxvueltas):
 ...
 for i in wp2: 
    ...
    while d > 3.:        
        ...
        + bd.vb
        bd.propulsion(delta,extf = - cd5.T[-2:],dfcm = [-(bd.ls/2.-0.13),0])   
#        bd.propulsion(delta)
        cd5.mtr_s(bd)
